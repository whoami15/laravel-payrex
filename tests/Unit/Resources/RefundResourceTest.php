...
it('creates a refund', function () {
    ...
    expect($result)->toBeInstanceOf(Refund::class)
        ->and($result->id)->toBe('re_ACubtXXN3XNRWpW3W1zQKzLWfs7Zn2sa')
        ...
        ->and($result->paymentId)->toBe('pay_BsubzXXN3XNRWpW3W1zPKzLWfszn3ssY')
    ...
    Http::assertSent(function ($r) {
        return $r->url() === 'https://api.payrexhq.com/refunds'
            && $r->method() === 'POST'
            && $r['payment_id'] === 'pay_BsubzXXN3XNRWpW3W1zPKzLWfszn3ssY'
            && $r['amount'] === 10000
            && $r['currency'] === 'PHP'
            && $r['reason'] === 'others';
    });
...
it('updates a refund', function () {
    Http::fake(['https://api.payrexhq.com/refunds/re_ACubtXXN3XNRWpW3W1zQKzLWfs7Zn2sa' => Http::response(loadFixture('refund/updated.json'))]);

    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');
    $result = $client->refunds->update('re_ACubtXXN3XNRWpW3W1zQKzLWfs7Zn2sa', [
    ...
    expect($result)->toBeInstanceOf(Refund::class)
        ->and($result->id)->toBe('re_ACubtXXN3XNRWpW3W1zQKzLWfs7Zn2sa')
        ->and($result->description)->toBe('Updated refund description')
        ->and($result->metadata)->toBe(['ticket_id' => 'SUP-001', 'resolved' => 'true']);

    Http::assertSent(fn ($r) => $r->url() === 'https://api.payrexhq.com/refunds/re_ACubtXXN3XNRWpW3W1zQKzLWfs7Zn2sa'
        && $r->method() === 'PUT'
    );
});
