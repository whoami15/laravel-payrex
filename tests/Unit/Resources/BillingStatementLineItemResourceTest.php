...
it('creates a billing statement line item', function () {
    ...
    expect($result)->toBeInstanceOf(BillingStatementLineItem::class)
        ->and($result->id)->toBe('bstm_li_BbSnRAFBA5S7DBJFuNBLFvsvLes11BSs')
        ...
        ->and($result->billingStatementId)->toBe('bstm_AbvnRnF9r577BBFFuNsLFvXvLes1CeeS')
        ->and($result->livemode)->toBeFalse();

    Http::assertSent(function ($r) {
        return $r->url() === 'https://api.payrexhq.com/billing_statement_line_items'
            && $r->method() === 'POST'
            && $r['billing_statement_id'] === 'bstm_AbvnRnF9r577BBFFuNsLFvXvLes1CeeS'
...
it('updates a billing statement line item', function () {
    Http::fake(['https://api.payrexhq.com/billing_statement_line_items/bstm_li_BbSnRAFBA5S7DBJFuNBLFvsvLes11BSs' => Http::response(loadFixture('billing_statement_line_item/updated.json'))]);

    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');
    $result = $client->billingStatementLineItems->update('bstm_li_BbSnRAFBA5S7DBJFuNBLFvsvLes11BSs', [
    ...
    Http::assertSent(function ($r) {
        return $r->url() === 'https://api.payrexhq.com/billing_statement_line_items/bstm_li_BbSnRAFBA5S7DBJFuNBLFvsvLes11BSs'
            && $r->method() === 'PUT'
...
it('deletes a billing statement line item', function () {
    Http::fake(['https://api.payrexhq.com/billing_statement_line_items/bstm_li_BbSnRAFBA5S7DBJFuNBLFvsvLes11BSs' => Http::response(loadFixture('billing_statement_line_item/deleted.json'))]);

    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');
    $result = $client->billingStatementLineItems->delete('bstm_li_BbSnRAFBA5S7DBJFuNBLFvsvLes11BSs');

    expect($result)->toBeInstanceOf(PayrexObject::class)
        ->and($result['id'])->toBe('bstm_li_BbSnRAFBA5S7DBJFuNBLFvsvLes11BSs')
        ->and($result['deleted'])->toBeTrue();

    Http::assertSent(fn ($r) => $r->url() === 'https://api.payrexhq.com/billing_statement_line_items/bstm_li_BbSnRAFBA5S7DBJFuNBLFvsvLes11BSs'
        && $r->method() === 'DELETE'
    );
});
