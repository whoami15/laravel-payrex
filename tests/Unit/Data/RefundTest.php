...
it('hydrates all properties from fixture', function () {
    ...
    expect($refund->id)->toBe('re_ACubtXXN3XNRWpW3W1zQKzLWfs7Zn2sa')
        ...
        ->and($refund->paymentId)->toBe('pay_BsubzXXN3XNRWpW3W1zPKzLWfszn3ssY')
        ->and($refund->livemode)->toBeFalse()
        ->and($refund->metadata)->toBeNull();
});

it('casts status to RefundStatus enum', function () {
    $refund = new Refund(['id' => 're_1', 'resource' => 'refund', 'status' => 'succeeded']);
    expect($refund->status)->toBe(RefundStatus::Succeeded);

    $refund = new Refund(['id' => 're_2', 'resource' => 'refund', 'status' => 'pending']);
    expect($refund->status)->toBe(RefundStatus::Pending);

    $refund = new Refund(['id' => 're_3', 'resource' => 'refund', 'status' => 'failed']);
    expect($refund->status)->toBe(RefundStatus::Failed);
});

it('casts reason to RefundReason enum', function () {
    $refund = new Refund(['id' => 're_1', 'resource' => 'refund', 'reason' => 'others']);
    expect($refund->reason)->toBe(RefundReason::Others);

    $refund = new Refund(['id' => 're_2', 'resource' => 'refund', 'reason' => 'requested_by_customer']);
    expect($refund->reason)->toBe(RefundReason::RequestedByCustomer);

    $refund = new Refund(['id' => 're_3', 'resource' => 'refund', 'reason' => 'fraudulent']);
    expect($refund->reason)->toBe(RefundReason::Fraudulent);
});

it('returns null for unknown enum values', function () {
    $refund = new Refund(['id' => 're_1', 'resource' => 'refund', 'status' => 'unknown', 'reason' => 'unknown']);
