...
it('hydrates all properties from fixture', function () {
    ...
    $statement = new BillingStatement($data);
...
it('casts status to BillingStatementStatus enum', function () {
    expect((new BillingStatement(['id' => 'bs_1', 'resource' => 'billing_statement', 'status' => 'draft']))->status)->toBe(BillingStatementStatus::Draft)
        ->and((new BillingStatement(['id' => 'bs_2', 'resource' => 'billing_statement', 'status' => 'open']))->status)->toBe(BillingStatementStatus::Open)
        ->and((new BillingStatement(['id' => 'bs_3', 'resource' => 'billing_statement', 'status' => 'paid']))->status)->toBe(BillingStatementStatus::Paid)
        ->and((new BillingStatement(['id' => 'bs_4', 'resource' => 'billing_statement', 'status' => 'void']))->status)->toBe(BillingStatementStatus::Void)
        ->and((new BillingStatement(['id' => 'bs_5', 'resource' => 'billing_statement', 'status' => 'uncollectible']))->status)->toBe(BillingStatementStatus::Uncollectible);
});

it('returns null for unknown status values', function () {
    $statement = new BillingStatement(['id' => 'bs_1', 'resource' => 'billing_statement', 'status' => 'nonexistent']);
