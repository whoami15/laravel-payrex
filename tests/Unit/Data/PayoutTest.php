...
it('casts status to PayoutStatus enum', function () {
    expect((new Payout(['id' => 'po_1', 'resource' => 'payout', 'status' => 'pending']))->status)->toBe(PayoutStatus::Pending)
        ->and((new Payout(['id' => 'po_2', 'resource' => 'payout', 'status' => 'in_transit']))->status)->toBe(PayoutStatus::InTransit)
        ->and((new Payout(['id' => 'po_3', 'resource' => 'payout', 'status' => 'failed']))->status)->toBe(PayoutStatus::Failed)
        ->and((new Payout(['id' => 'po_4', 'resource' => 'payout', 'status' => 'successful']))->status)->toBe(PayoutStatus::Successful);
});

it('returns null for unknown status values', function () {
    $payout = new Payout(['id' => 'po_1', 'resource' => 'payout', 'status' => 'nonexistent']);
