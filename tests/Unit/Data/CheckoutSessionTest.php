...
it('hydrates all properties from fixture', function () {
    ...
    $session = new CheckoutSession($data);
...
it('casts status to CheckoutSessionStatus enum', function () {
    $session = new CheckoutSession(['id' => 'cs_1', 'resource' => 'checkout_session', 'status' => 'active']);
    expect($session->status)->toBe(CheckoutSessionStatus::Active);

    $session = new CheckoutSession(['id' => 'cs_2', 'resource' => 'checkout_session', 'status' => 'completed']);
    expect($session->status)->toBe(CheckoutSessionStatus::Completed);

    $session = new CheckoutSession(['id' => 'cs_3', 'resource' => 'checkout_session', 'status' => 'expired']);
    expect($session->status)->toBe(CheckoutSessionStatus::Expired);
});

it('returns null for unknown status values', function () {
    $session = new CheckoutSession(['id' => 'cs_1', 'resource' => 'checkout_session', 'status' => 'nonexistent']);
