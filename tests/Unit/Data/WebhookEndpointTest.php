...
it('hydrates all properties from fixture', function () {
    ...
    $webhook = new WebhookEndpoint($data);
...
it('casts status to WebhookEndpointStatus enum', function () {
    expect((new WebhookEndpoint(['id' => 'wh_1', 'resource' => 'webhook', 'status' => 'enabled']))->status)->toBe(WebhookEndpointStatus::Enabled)
        ->and((new WebhookEndpoint(['id' => 'wh_2', 'resource' => 'webhook', 'status' => 'disabled']))->status)->toBe(WebhookEndpointStatus::Disabled);
});

it('returns null for unknown status values', function () {
    $webhook = new WebhookEndpoint(['id' => 'wh_1', 'resource' => 'webhook', 'status' => 'nonexistent']);
