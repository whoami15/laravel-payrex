...
it('casts status to PaymentStatus enum', function () {
    $payment = new Payment(['id' => 'pay_1', 'resource' => 'payment', 'status' => 'paid']);

    expect($payment->status)->toBe(PaymentStatus::Paid);
});

it('casts failed status to PaymentStatus enum', function () {
    $payment = new Payment(['id' => 'pay_1', 'resource' => 'payment', 'status' => 'failed']);

    expect($payment->status)->toBe(PaymentStatus::Failed);
});

it('returns null for unknown status values', function () {
    $payment = new Payment(['id' => 'pay_1', 'resource' => 'payment', 'status' => 'nonexistent_status']);
...
it('hydrates customer as Customer when expanded', function () {
    ...
    expect($payment->customer)->toBeInstanceOf(Customer::class)
        ->and($payment->customer->id)->toBe('cus_1');
});
